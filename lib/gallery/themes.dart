...
ThemeData _buildLightTheme() {
  ...
  final ThemeData base = ThemeData(
    ...
    // splashColor: Colors.white24,// 水波动画的颜色，可通过设置为透明色来取消水波动画
    ...
    errorColor: const Color(0xFFB000020),
    buttonTheme: ButtonThemeData(
      colorScheme: colorScheme,
      textTheme: ButtonTextTheme.primary,
    ),
...
* brightness: 应用程序整体主题的亮度。
    由按钮等小部件使用，以确定在不使用主色或强调色时要选择的颜色。
    当亮度较暗时，画布，卡片和原色都是黑暗的。
    当亮度很亮时，画布和卡片的颜色很���亮，主色的暗度会因primaryColorBrightness的描述而变化。
    当亮度较暗时，primaryColor与卡片和画布颜色形成鲜明对比;
    当亮度较暗时，使用Colors.white或accentColor作为对比色。

* colorScheme：: 一组13种颜色，可用于配置大多数组件的颜色属性。
    此属性的添加时间远远超过主题的高度特定颜色集，如cardColor，buttonColor，canvasColor等。
    新组件只能根据colorScheme定义。现有组件将逐渐迁移到它，在可能的范围内没有显着的向后兼容性中断。

* dividerColor: Dividers和PopupMenuDividers的颜色，也在ListTiles之间，DataTable中的行之间使用，等等。
    要创建使用此颜色的适当BorderSide，请考虑 Divider.createBorderSide。

* inputDecorationTheme: InputDecorator，TextField和TextFormField的默认InputDecoration值基于此主题。
    请参阅InputDecoration.applyDefaults。

* pageTransitionsTheme: 每个TargetPlatform的默认MaterialPageRoute转换。
   MaterialPageRoute.buildTransitions委托给PageTransitionsBuilder
   其PageTransitionsBuilder.platform相匹配的平台。如果未找到匹配的构建器，则使用其平台为null的构建器。

* platform: 材料小部件应该适应目标的平台。 默认为当前平台。这应该用于根据平台约定来设置UI元素的样式。
    Platform.defaultTargetPlatform应该直接使用，而只是在极少数情况下，需要根据平台确定行为。
    dart.io.Platform.environment当实际知道当前平台至关重要时，应该使用它，
    而不需要任何覆盖（例如，当即将调用系统API时）。

* splashFactory: 定义InkWell 和InkResponse产生的墨水飞溅的外观。
   也可以看看： 1.InkSplash.splashFactory，定义默认的splash。
   2.InkRipple.splashFactory，它定义了一个比默认值更积极地展开的splash。
